...
<?php
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'add'){
        ...
        if($cekID2 == null){
            mysqli_query($db, "INSERT INTO entered (id_enter, penginput, nisn_s, nisn_j, sanksi, created_at) VALUES (null, '$namaSiswa', null, null, null, now())");
            ...
            mysqli_query($db, "UPDATE entered SET penginput = '$namaSiswa', nisn_s = null, nisn_j = null, sanksi = null, created_at = now() WHERE id_enter = '$cekID4'");
    ...
    if(isset($_POST['simpan'])){
        ...
        if (!empty($idEnter) && !empty($nisnJunior) && !empty($nilaiSikap) && !empty($nilaiPolaPikir) && !empty($nilaiKeaktifan) && !empty($nilaiPBB) && isset($pelanggaran)) {
            ...
            if($input1 && $input2 && $input3 && $input4){
                ...
                $inputEntered = mysqli_query($db, "UPDATE entered SET penginput = '$namaSiswa', nisn_s = null, nisn_j = '$nisnJunior', sanksi = '$pelanggaran', created_at = NOW() WHERE id_enter = '$idEnter'");

                //membuat value baru di entered untuk data nilai selanjutnya

                if($inputEntered){
                    $queryInputNewId = mysqli_query($db, "INSERT INTO entered (id_enter, penginput, nisn_s, nisn_j, sanksi, created_at) VALUES (null, '$namaSiswa', null, null, null, now())");
